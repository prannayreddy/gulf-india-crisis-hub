...
(function () {
  ...
  function formatFeedTime(date, time) {
    // Handles both "2026-03-02" + "04:38" and issuedDate "2026-03-02" (no time)
    if (!date) return '';
  ...
  function renderFeed(data) {
    ...
    sorted.forEach(function (item, i) {
      ...
      html += '<span class="feed-item__time">' + formatFeedTime(item.date || item.issuedDate, item.time) + '</span>';
      html += '</div>';
